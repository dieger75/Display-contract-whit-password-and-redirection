<?php
/****************************************************
 * Display contract whit password and redirection
 ***************************************************/
...
/********************************************
 * Display contract whit password
 *******************************************/
add_action('template_redirect', 'verify_external_session_and_access_contract');

function verify_external_session_and_access_contract() {
    // Solo ejecutar si estamos en la página del contrato y se proporciona el parámetro de verificación
    if (isset($_GET['verification']) && $_GET['verification'] == 'true') {
        error_log('Iniciando verificación y acceso a la página de contrato...');

        // Verificar si el usuario tiene una sesión externa activa (basado en la cookie de sesión externa)
        if (isset($_COOKIE['external_session_id'])) {
            error_log("Acceso permitido: Sesión externa detectada para el usuario.");

            // En este punto, se ha verificado la sesión externa, por lo tanto, insertamos la contraseña en el formulario de la página de contrato.

            // Definir la contraseña que se va a insertar automáticamente
            $password = 'eRY3j8Vp0#wP@0FoTs)k!rFaqPsaG@^Rx@eow$JwcLqwS5@j';

            // Insertar el script en el pie de página (wp_footer)
            ...
            // No hay sesión externa válida, redirigir al login
            error_log("Acceso denegado: Sesión externa no válida o expirada.");
            wp_redirect(site_url('/user-account?token=invalid_or_expired'));
            exit();
        }
    }
}
